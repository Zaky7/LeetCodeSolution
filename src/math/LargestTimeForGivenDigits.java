...
class LargestTimeForGivenDigits {
  ...
  public String largestTimeFromDigits(int[] A) {
    ...
    for (List permutation : permutations) {
      int hours = castToTime(
        (Integer) permutation.get(0),
        (Integer) permutation.get(1)
      );
      int minutes = castToTime(
        (Integer) permutation.get(2),
        (Integer) permutation.get(3)
      );
  ...
  private void getAllPermutationUtil(
    int start,
    List<Integer> list,
