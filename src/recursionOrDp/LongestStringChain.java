...
class LongestStringChain {
  ...
  public int longestStrChain(String[] words) {
    ...
    for (int i = 1; i < n; i++) {
      ...
      for (int j = 0; j < i; j++) {
        if (isDifferByOne(words[i], words[j])) out.println(
          words[i] +
          " " +
          words[j] +
          "by " +
          isDifferByOne(words[i], words[j]) +
          " and is predecessor: " +
          isPredecessor(words[i], words[j])
        );

        if (
          isDifferByOne(words[i], words[j]) && isPredecessor(words[i], words[j])
        ) {
          chainLengthArr[i] =
            Math.max(chainLengthArr[i], 1 + chainLengthArr[j]);
        }
      }
    }
