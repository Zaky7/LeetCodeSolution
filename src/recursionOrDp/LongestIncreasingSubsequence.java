...
class LongestIncreasingSubsequence {
    ...
    public int LIC(int[] arr) {
        ...
        for(List<Integer> temp : subsequenceList) {
            if(isIncreaseSubsequence(temp)) {
                System.out.println(temp.toString() + " is increasing: " + isIncreaseSubsequence(temp));
    ...
    private void LICUtil(int[] arr, int start, List<Integer> subsequence, List<List<Integer>> subsequenceList) {
        if(start >= arr.length) {
